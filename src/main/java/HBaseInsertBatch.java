...
public class HBaseInsertBatch {
	...
	private static final String USAGE = "HBase batch write directly performace test.\n"
					+ " Usage:  <arg1> <arg2> <arg3> <arg4> <arg5> [<arg6>...].\n"
					+ " Or quit(Ctrl-C) then input : java -cp HBaseTest.jar HBaseInsertBatch <arg1> <arg2> <arg3> <arg4> <arg5> [<arg6>...].\n"
    ...
    public static void main(String[] args) {
		...
		final AtomicLong seqNum = new AtomicLong(startKey);
					...
					for (int i = 0; i < clientThreads; i++) {
						executor.execute(new Runnable() {
							...
							public void run() {
		                        ...
		                        while (true) {
		                            long end = seqNum.addAndGet(batch);
		                            long start = end - batch;
		                            for (long rowKey = start+1L; rowKey <= end; rowKey++) {
		                            ...
		                            try {
										...
										if ((System.currentTimeMillis() - begin) > warningThreshold) {
											warningCount.addAndGet(batch);	
										}
		                                successCount.addAndGet(batch);		                             
		                                list.clear();
		                            } catch (Exception e) {
		                            	errorCount.addAndGet(batch);
		                                e.printStackTrace();
		                            }
		                        }
							}
        ...
        while (seqNum.get() < (endKey-startKey)) {
			...
			System.out.println("ClientCount: ["+ clientCount + "], ThreadNum/perClient: ["+clientThreads + "], "
							+ "per second: [" + (int)((success2-success1)/batch) + "], tps: [" + (int) (1000.0 * success2 / (cur-start)*batch) + "]"
    ...
    public static void scanInput(String[] args) {
    	...
    	try {
	    	if(args.length >= 5) {
	    		...
	    		batch = args.length>5 ? Integer.parseInt(args[5]) : 1;
	    		startKey = args.length>6 ? Long.parseLong(args[6]) : 0L;
	    		endKey = args.length>7 ? Long.parseLong(args[7]) : Long.MAX_VALUE;
	    		warningThreshold = args.length>8 ? Integer.parseInt(args[8]) : 100;
	    		return;
	    	}
		} catch(Exception e) {
    	...
    	try {
        	while(true) {
	    		...
	    		Pattern p = Pattern.compile("(\\S+)\\s+(\\S+)\\s+(\\S+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)?\\s+(\\d+)?\\s+(\\d+)?\\s+(\\d+)?", Pattern.DOTALL);
	        	...
	        	batch = (m.group(6)!=null) ? Integer.parseInt(m.group(6)) : 1;
	        	startKey = (m.group(7)!=null) ? Long.parseLong(m.group(7)) : 0L;
	        	endKey = (m.group(8)!=null) ? Long.parseLong(m.group(8)) : Long.MAX_VALUE;
	    		warningThreshold = (m.group(9)!=null) ? Integer.parseInt(m.group(9)) : 100;
	    		return;
        	}
    	} catch(Exception e) {
    		e.printStackTrace();
